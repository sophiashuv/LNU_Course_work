...
def model_predict(model, treshhold=0.5):
    ...
    X_t = cv2.resize(X_train[ix], (5456, 3632))
    plt.imshow(np.squeeze(Y_train[ix]))
    plt.show()
    Y_t = cv2.resize(np.squeeze(Y_train[ix]), (128, 128), interpolation = cv2.INTER_AREA)
    plt.imshow(Y_t)
    plt.show()
    P_t = cv2.resize(np.squeeze(preds_train_t[ix]), (5456, 3632))
    ...
    plot2, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(18, 5))
    X_v = cv2.resize(X_train[int(X_train.shape[0] * 0.9):][ix], (5456, 3632))
    Y_v = cv2.resize(np.squeeze(Y_train[ix]), (5456, 3632))
    P_v = cv2.resize(np.squeeze(preds_val_t[ix]), (5456, 3632))
    ax1.imshow(X_v)
    ax2.imshow(Y_v)
    ax3.imshow(P_v)
    ...
    X_t = cv2.resize(X_test[ix], (5456, 3632))
    P_t = cv2.resize(np.squeeze(preds_test_t[ix]), (5456, 3632))
    ax1.imshow(X_t)
    ax2.imshow(P_t)
