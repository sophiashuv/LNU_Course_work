...
def plot_matrices(models, x, ys, y_value):
...
def plot_average(matrices):
    y = ["iou", "accuracy", "precision", "recall", "mcc"]
    colors = ["#9FA8DA", "#81D4FA", "#80CBC4", "#C5E1A5", "#FF8A65"]
    matrices.plot(x="MODEL", y=y, color=colors, kind="bar", zorder=3, figsize=(18, 9))
...
def plot_comparison(task):
    if task == '1':
        models, x, ys = get_image_matrices(MATRICES_PATH, metric, 20, THRESHOLD)
        plot_matrices(models, x, ys, metric)
    elif task == '2':
        matrices = get_model_avg(MATRICES_PATH, THRESHOLD)
        plot_average(matrices)
    elif task == '3':
...
if __name__ == '__main__':
    ...
    parser.add_argument("--task",
                        default='3',
                        help='1 - plot_matrices, 2 - plot_average')
